...
(define (sbir-print element)
   (if (not (null? element))
      (begin
         (if (string? (car element))
             (display (car element))
             (display (evalexpr (car element)))
         )
         (sbir-print (cdr element))
      )
      (newline))
)
   ;(display (cddddr element))
 ...
 ;        (newline)
  ...
  ;    (when (and (= (length element) 2) (number? (cadr element))) ; if printing a number
   ;      (display (cadr element)) ; print out the number
    ...
    ;  (when (and (or (= (length element) 3) (= (length element) 2)) (string? (cadr element))) ; if printing a string
...
; takes in numeric values. for each value read into a variable, the value is
; inserted into the Symbol table under that variable's key. For arrays, the
; array must already exist and the subscript not be out of bounds.
; has variable inputcount which stores the number of values successfully read
(define (sbir-input invar) ; input variables
   ...
   (when (not (null? (car invar)))
     ...
     ;(display (hash-ref *variable-table* 'inputcount))
      (let ((obj (read))) ; read in and store in object
         (cond ((eof-object? obj) (hash-set! *variable-table* 'inputcount -1)) ; if end of file, set inputcount to -1
               ((number? obj) (hash-set! *variable-table* (car invar) obj) ; if obj is a number, set it to the input variable
                                 (hash-set! *variable-table* 'inputcount (+ (hash-ref *variable-table* 'inputcount) 1)))
...
(for-each
  ...
  `(
      (/       ,(lambda (x y) (/ x  (+ 0.0 y)))) ;always add 0.0 before dividing
  ...
  (map (lambda (line)
     (when (not (null? line)) ; when the line is not null
        (when (= 3 (length line)) ; if the length of the line is 3
           (hash-set! *label-table* (cadr line) (- (car line) 1))) ; hash-set to the label table
        (when (and (= 2 (length line)) (not (list? (cadr line))))  ; if there is only one element and it isn't a list, then it is a label
             (hash-set! *label-table* (cadr line) (- (car line) 1))) ; hash-set it to the label table
             )) program)
...
(when (> (length program) linenum)
   (let ((line (list-ref program linenum))) ;let line be the corresponding linenum
    ...
    (when (not (null? (cdr line)))
       (when (and (not (null? (cadr line))) (>= (length line) 2) (list? (cdr line)))
      (let ((funst (list-ref line (- (length line) 1))))
         (when (and (not (null? (cadr line))) (>= (length funst) 2))
            ...
            (cond ((equal? func 'print) (sbir-print funst)
                                        (evaluate-line program (+ linenum 1))) ; if the function is print, send the whole function + statement
                                        
                  ((equal? func 'let) (sbir-let state)
                                      (evaluate-line program (+ linenum 1)))
                                        
                  ((equal? func 'goto) (sbir-goto program state)) ; dont evaluate next line if goto

                  ((equal? func 'if) (if (evalexpr (car state)) ; evaluate to see if the condition is true
                                     (sbir-goto program (cdr state)) ; goto the program 
                                     (evaluate-line program (+ linenum 1))))
